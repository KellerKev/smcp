...
class SMCPAgent(SMCPNode):
    ...
    def _execute_task(self, task: Task, agent: AgentInfo) -> Any:
        """Execute a task using a real registered capability handler.

        Dispatches ``task.type`` to a handler in ``self.tool_handlers``
        (populated by ``register_capability``) and returns its actual result. If
        no handler is registered for the task type, returns an explicit
        ``no_handler`` result rather than fabricating analysis/research output —
        an honest signal that this node can't do the work, not fake success.
        """
        self.registry.update_agent_load(agent.agent_id, agent.load + 0.1)

        handler = getattr(self, "tool_handlers", {}).get(task.type)
        if handler is not None:
            payload = task.input_data if isinstance(task.input_data, dict) else {"input": task.input_data}
            try:
                return handler(**payload)
            except TypeError:
                # Handler signature doesn't match the payload kwargs; pass the
                # whole payload as a single argument instead.
                return handler(payload)
        ...
        return {
            ...
            "task_type": task.type,
            "agent": agent.name,
            "detail": f"No registered handler for task type '{task.type}' on agent '{agent.name}'",
        }

    # Backwards-compatible name used by the distributed layer and workflow code.
    _simulate_task_execution = _execute_task

    def _collaborative_contribution(self, agent: AgentInfo, task: str, data: Dict[str, Any]) -> Any:
        """A single agent's contribution to a collaboration.

        Dispatches to a real handler when one matches the agent's specialty,
        otherwise returns an honest structured note (no fabricated sources or
        findings)."""
        for specialty in agent.specialties:
            handler = getattr(self, "tool_handlers", {}).get(specialty)
            if handler is not None:
                try:
                    return handler(task=task, data=data)
                except TypeError:
                    return handler({"task": task, "data": data})
        return {
            "status": "no_handler",
            "contribution_by": agent.name,
            "specialties": agent.specialties,
            "task": task,
        }

    # Backwards-compatible name.
    _simulate_collaborative_work = _collaborative_contribution
